...
import com.example.android.abr_predator.R;
        ...
        CvCameraViewListener2 // implements IOIOLooperProvider: from IOIOActivity
{
	...
	public final void onSensorChanged(SensorEvent event) {
		 if(m_ioio_thread != null){
			  setText("sonar1: "+m_ioio_thread.getIrLeftReading(), sonar1Text);
			  setText("sonar2: "+m_ioio_thread.getIrCenterReading(), sonar2Text);
			  setText("sonar3: "+m_ioio_thread.getIrRightReading(), sonar3Text);
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		...
		*/
		...
		if (autoMode) {    //search if autoMode is on
			...
			//Log.i("GREEN",Float.toString(greenRadius[0]));
			Log.i("YellowRadius",Float.toString(yellowRadius[0]));

			centerDifference = mDetectorYellow.getCenterDifference();
			Log.i("yellowMoment",String.format("%.2f", centerDifference));
			...
			else if (yellowCenter.x  < 500 && (yellowRadius[0] < 320 && yellowRadius[0] > 70)) {
				m_ioio_thread.move(1500); //stop
				m_ioio_thread.turn(1400); //turn ccw
			}
